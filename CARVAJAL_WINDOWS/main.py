...
# Para usar Playwright, es necesario instalar los navegadores compatibles:
# Ejecuta el siguiente comando en tu terminal después de instalar Playwright:
# playwright install
...
from notificaciones import enviar_notificacion_error, enviar_notificacion_exito
...
# Credenciales (usar variables de entorno si están disponibles)
USUARIO = os.getenv('USUARIO_PORTAL', "director.cadenas@dismelltda.com")
CONTRASENA = os.getenv('CONTRASENA_PORTAL', "Dismel05211921#$")
...
DIRECTORIO_LOGS = os.getenv('DIRECTORIO_LOGS', "logs_reportes")
                                ...
                                if boton_descargar_modal:
                                    ...
                                    try:
                                        with page.expect_download() as download_info:
                                            ...
                                            # Crear subcarpeta por tipo de reporte si no existe
                                            subcarpeta = os.path.join(DIRECTORIO_DESCARGAS, report_type.lower())
                                            Path(subcarpeta).mkdir(exist_ok=True)
...
def main():
    try:
        with sync_playwright() as p:
            ...
            while intentos < MAX_INTENTOS and not login_exitoso:
                ...
                try:
                    ...
                    # Determinar qué descargar basado en argumentos de línea de comandos
                    tipo_descarga = sys.argv[1].upper() if len(sys.argv) > 1 else "AMBOS"
                    
                    try:
                        # Descargar VENTAS si se solicita
                        if tipo_descarga in ["VENTAS", "AMBOS"]:
                        ...
                        # Descargar INVENTARIO si se solicita
                        if tipo_descarga in ["INVENTARIO", "AMBOS"]:
                            descargar_reportes_con_paginacion(page, RE_INVENTARIO, "INVENTARIO")
                            logger.info("✅ Descarga de reportes de INVENTARIO completada exitosamente")
                        
                        # Validar parámetro
                        if tipo_descarga not in ["VENTAS", "INVENTARIO", "AMBOS"]:
                            logger.warning(f"⚠ Parámetro no reconocido: {tipo_descarga}. Opciones: ventas, inventario, ambos")
                        ...
                        enviar_notificacion_error(
                            ...
                            descripcion_error=f"Se presentó un error al intentar descargar los reportes del portal Cencarvajal.",
                            tipo_error="DESCARGA",
                            paso_fallido=f"Descarga de reportes de {tipo_descarga}",
        ...
        # 📧 DESCOMENTA ESTA SECCIÓN SI DESEAS RECIBIR NOTIFICACIÓN DE ÉXITO POR CORREO:
        # enviar_notificacion_éxito(
        #     cantidad_descargas=9,  # Reemplaza con el número real de descargas
        #     ruta_reportes=DIRECTORIO_DESCARGAS,
        #     ruta_logs=archivo_log
        # )
        # logger.info("✉️ Notificación de éxito enviada por correo")
